...
<script>
	$(document).ready(function() {
		...
		$('[id^="editReview_"]').click(function(event) {
		    ...
		    $(this).closest('.review_box').addClass('hide');
		    
		    let reviewContents = $('#review_' + revNum).find('[name=review_contents]').text();
		    $('#review_' + revNum).find('[name=review_contents]').val(reviewContents.replace(/<br\s*\/?>/gi, '\n'));
		    $('#review_' + revNum).closest('.review_box').removeClass('hide');
		});
	    ...
	    $('[id^="close_"]').click(function(event) {
	    	...
	    	let revNum = this.id.split("_").pop();
	    	$(this).closest('.review_box').addClass("hide");
	    	$(this).closest('.review_box').prev().removeClass('hide');
	    });
	    ...
	    // 후기 작성 Form submit시 발생
	  	$("#reviewForm").submit(function(event) {
	  		...
	  		// 줄바꿈을 <br/>로 저장
	  	  $(this).find('[name=review_contents]').val($(this).find('[name=review_contents]').val().replace(/\n/gi, '<br/>'));
	  	});
		...
		<c:forEach items = "${revList}" var = "dto">
				...
				<c:param name = "num" value = "${dto.review_keynum}"/>
	                   ...
	                   <div class="review_box">
	                       ...
	                       <p class="review_text">
	                      	 ${dto.review_contents}
	                           <br />
	                           <span>작성자: ${dto.user_id}&nbsp;&nbsp;·&nbsp;&nbsp;${dto.review_date }</span>
	                           <c:if test="${sessionScope.authInfo != null && sessionScope.authInfo.user_id == dto.user_id }">
	                           <form id="EditDeleteFrm_${dto.review_keynum }" method = "POST" action = "/bookList/delete">
		                           <span>&nbsp;&nbsp;·&nbsp;&nbsp;<a href="" id="editReview_${dto.review_keynum }">수정</a></span>
		                           <input type="hidden" name="review_keynum" value="${dto.review_keynum }" />
		                           <span>&nbsp;&nbsp;·&nbsp;&nbsp;<a href="" id="deleteReview_${dto.review_keynum }">삭제</a></span>
			...
			<c:if test="${sessionScope.authInfo != null && sessionScope.authInfo.user_id == dto.user_id }">
              <div class="review_box hide">
				<form id = "review_${dto.review_keynum}" class="updateReview" method = "POST" action = "update">
			                        <ul>
						                <li>
						                    <h4>리뷰 수정
