...
<%@ page import="user.map.rea"%>
<%@ page import="user.map.getmapcenter"%>
<%@ page import="user.map.GetAddress"%>
<%
rea rea = new rea();
...
<table>
	<thead>
		<tr>
			<th style="text-align: center;">도서관 이름</th>
			<th style="text-align: center;">지번 주소</th>
			<th style="text-align: center;">도로명 주소</th>
			<th style="text-align: center;">전화번호</th>
			<th style="text-align: center;">카카오지도페이지</th>

		</tr>
	</thead>
	<tbody>
		<%
		// tbody에 추가될 행(row)의 개수

		// numRows개의 행(row) 추가
		for (int i = 0; i < 15; i++) {
		%>
		<tr>
			<td><%=rea.GetNamelist().get(i).toString()%></td>
			<td><%=rea.GetAddresslist().get(i).toString()%></td>
			<td><%=rea.GetRoadlist().get(i).toString()%></td>
			<td><%=rea.GetPhonelist().get(i).toString()%></td>
			<td><%=rea.GetUrllist().get(i).toString()%></td>
		</tr>
		<%
		}
		%>
...
<script>
	var mapcentery =
<%=rea.GetCentery().get(0)%>
	;
	var mapcenterx =
<%=rea.GetCenterx().get(0)%>
	;
	var xlist =
<%=rea.GetXlist()%>
	;
...
<%=(String) centery.get(0)%>
	,
<%=(String) centerx.get(0)%>
	), // 지도의 중심좌표
		level : 3
	// 지도의 확대 레벨
	};
	...
	function getListItem(index) {
		...
		itemStr += '  <span class="tel">' + phonelist[i] + '</span>' + '</div>';
